...
constexpr std::string_view kRawAlertFields = R"(
    id,
    target_id,
    type,
    severity,
...
constexpr std::string_view kFormatTimestampSql =
    R"(YYYY-MM-DD"T"HH24:MI:SS.MS"Z")";
...
Alert AlertRepository::CreateAlertWithEvent(
    const NewAlert& alert, const AlertEventTargetSnapshot& target) const {
  const auto result =
      pg_cluster_->Execute(
          userver::storages::postgres::ClusterHostType::kMaster,
          R"(
            WITH event_metadata AS (
                SELECT gen_random_uuid()::TEXT AS event_id
            ),
            created_alert AS (
                INSERT INTO alerts (
                    target_id,
            ...
            outbox_event AS (
                ...
                SELECT
                    ...
                    $5,
                    created_alert.id,
                    created_alert.target_id::TEXT,
                    jsonb_build_object(
                        'version', 1,
                        'event_id', event_metadata.event_id,
                        'event_type', $5,
                        'occurred_at', to_char(created_alert.created_at AT TIME ZONE 'UTC', $8),
                        'producer', 'alert-service',
                        'alert', jsonb_build_object(
                            'id', created_alert.id,
                            'target_id', created_alert.target_id,
                            'type', created_alert.type,
...
std::optional<Alert> AlertRepository::ResolveActiveAlertWithEvent(
          ...
          R"(
            ...
            outbox_event AS (
                ...
                SELECT
                    ...
                    $3,
                    resolved_alert.id,
                    resolved_alert.target_id::TEXT,
                    jsonb_build_object(
                        'version', 1,
                        'event_id', event_metadata.event_id,
                        'event_type', $3,
                        'occurred_at', to_char(resolved_alert.resolved_at AT TIME ZONE 'UTC', $6),
                        'producer', 'alert-service',
                        'alert', jsonb_build_object(
                            'id', resolved_alert.id,
                            'target_id', resolved_alert.target_id,
                            'type', resolved_alert.type,
