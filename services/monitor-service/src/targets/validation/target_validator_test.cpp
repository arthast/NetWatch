...
UTEST(TargetValidator, RejectsHttpTargetWithoutUrl) {
    ...
    const auto error = monitor_service::target_validator::ValidateCreateTargetRequest(request);
...
UTEST(TargetValidator, RejectsTimeoutGreaterThanInterval) {
    ...
    ASSERT_TRUE(error.has_value());
    EXPECT_EQ(*error, "timeout_ms must be less than interval_seconds in milliseconds");
}
